...
class FTP:
    ...
    def connect(self, host='', port=0, timeout=-999, source_address=None, ignorepasvaddress=False):
        '''Connect to host.  Arguments are:
         ...
         - ignorepasvaddress: ignore ip address received from server after command 'PASV' 
           and using original ip address
        '''
        if host != '':
            self.host = host
        if port > 0:
        ...
        if source_address is not None:
            self.source_address = source_address
        if ignorepasvaddress:
            self.ignorepasvaddress = True 
